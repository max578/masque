...
S7::method(print, masque_recipe) <- function(x, ...) {
  ...
  cli::cli_text("")
  cli::cli_alert_danger("PRIVATE - never share this recipe alongside the synthetic.")
  cli::cli_text("Use {.code reveal_maps(rec)} to inspect level maps explicitly.")

  invisible(x)
}

S7::method(format, masque_recipe) <- function(x, ...) {
  sprintf("<masque_recipe: mode=%s, cols=%d, level-maps=%d>",
...
S7::method(print, masque) <- function(x, ...) {
  ...
  cli::cli_bullets(c(
    "*" = sprintf("Mode: %s", x@mode),
    "*" = sprintf("Synthetic: %d row(s) x %d column(s)",
                  nrow(x@synthetic), ncol(x@synthetic)),
    "*" = sprintf("Audit: %s",
                  if (is.null(x@audit)) "not run (use audit_mask())" else
                    sprintf("%d row(s)", nrow(x@audit)))
  ))

  if (identical(x@mode, "local")) {
    cli::cli_alert_warning("local mode: synthetic data is for owner development only, not external sharing.")
  }

  cli::cli_text("Use {.code synthetic(m)} to extract data; {.code recipe(m)} for the recipe.")
  cli::cli_text("")
  print(x@recipe)
...
reveal_maps <- function(rec) {
  ...
  cli::cli_alert_warning("Revealing sensitive level maps. Proceed at your discretion.")
  cli::cli_text("")
