...
synthesise_numeric_conditional <- function(x_num, groups, min_stratum = 5L) {
  ...
  # Resolve strata and identify the ones too small to synthesise on their
  # own. A row whose group label is NA is never poolable into a real
  # stratum, so it joins the fallback as well.
