...
.compute_audit <- function(df, synth, rec, mode) {
  ...
  rows <- lapply(cols, function(col) {
    ...
    alias_status <- if (has_map) "aliased" else if (!in_synth) "dropped" else "passthrough"
    ...
    notes <- .audit_notes(role, kind, pii, alias_status, mode, leakage_class,
...
.classify_leakage <- function(role, kind, pii, mode, in_synth, alias_status,
                              exact_match_pct, freq_min, n_unique_levels, n_rows) {
  # 1. PII pattern retained as not-ignored: HIGH
  if (pii && role != "ignore" && in_synth) return("high")

  # 2. Treatment unaliased in collaborate: HIGH
  if (role == "treatment" && mode == "collaborate" && alias_status == "passthrough") {
    return("high")
  }
  ...
  if (role == "covariate" && kind %in% c("numeric", "integer") &&
      mode == "collaborate" && !is.na(exact_match_pct) && exact_match_pct > 5) {
    return("medium")
  }

  # 6. Ignore column retained in local: LOW (informational)
  if (role == "ignore" && mode == "local" && in_synth) return("low")
...
.audit_notes <- function(role, kind, pii, alias_status, mode, leakage_class,
  ...
  if (alias_status == "passthrough" && role == "treatment" && mode == "collaborate") {
    bits <- c(bits, "treatment passthrough in collaborate (unexpected)")
  }
  if (!is.na(freq_min) && freq_min == 1L && role == "covariate" && mode == "collaborate") {
    bits <- c(bits, "rare level (freq = 1)")
  }
  if (!is.na(exact_match_pct) && exact_match_pct > 1 && mode == "collaborate") {
    bits <- c(bits, sprintf("exact-match %.1f%% (jitter due step 7)", exact_match_pct))
  }
