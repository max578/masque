...
test_that("audit_mask flags freq=1 categorical covariate as HIGH in collaborate", {
  set.seed(0)
  df <- data.frame(
    Rep   = rep(1:4, 25),
    rare  = factor(c("only_one", sample(c("alpha","beta"), 99, replace = TRUE))),
    yield = rnorm(100),
    stringsAsFactors = FALSE
  )
  r <- propose_roles(df)
  r$role[r$col == "yield"] <- "outcome"
  expect_warning(m <- mask(df, r, mode = "collaborate", seed = 1), "HIGH leakage")
  out <- audit_mask(m, print = FALSE)
...
test_that("Audit reports a numeric exact-match-pct for the outcome under collaborate", {
...
test_that("audit_mask prints a styled report by default and returns invisibly", {
  df <- make_audit_fixture()
  r  <- basic_roles(df)
  m  <- mask(df, r, mode = "collaborate", seed = 1)
