...
`mask()`, `apply_recipe()`, and `unmask()` unchanged. Date/time covariates
are different: `mask()` row-permutes them in the synthetic, while
`apply_recipe()` leaves the original values unchanged because the pipeline
is being retargeted to the original data.
