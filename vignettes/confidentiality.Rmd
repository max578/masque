...
4. **Never override `pii_suspected = TRUE` without thinking.** The
   ...
   to keep the column.
5. **Use `keep` sparingly.** It is there for columns your pipeline needs
   exactly as-is. In collaborate mode it is still published exactly as-is.
6. **Date/time columns carry real operational signal.** They are no
   longer dropped by default; they are row-permuted as covariates. Re-role
   to `keep` only when exact row alignment is required, or to `ignore` if
   the schedule itself is sensitive.
