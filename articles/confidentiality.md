...
| Synthetic + external side info (e.g., publicly registered trial) | Identity of treatments / sites / intentionally kept metadata | Only the label vocabulary; the design footprint and any `keep` columns are preserved by design and are recognisable. Side info wins. |
...
| Date/time covariates | row-permuted; class preserved | row-permuted; class preserved |
| `keep` columns | pass-through | pass-through |
...
4.  **Never override `pii_suspected = TRUE` without thinking.** The
    ...
    to keep the column.
5.  **Use `keep` sparingly.** It is there for columns your pipeline
    needs exactly as-is. In collaborate mode it is still published
    exactly as-is.
6.  **Date/time columns carry real operational signal.** They are no
    longer dropped by default; they are row-permuted as covariates.
    Re-role to `keep` only when exact row alignment is required, or to
    `ignore` if the schedule itself is sensitive.
7.  **Small designs leak.** Cell counts of 1 in any categorical column
    under collaborate mode are HIGH leakage. Aggregate or drop before
    masking.
8.  **Exact-design preservation is recognisable.** A publicly registered
