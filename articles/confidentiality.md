...
**A coordinate is masked unless you say otherwise.** Dropping is one way
to say what happens to it; there are three, and
...
with `action = "keep"`, the call stops. Declare the pair to `coords` and
it is coarsened, give the column `drop` or `scramble` and it is masked,
or pass `allow_unmasked_coords = TRUE` having decided the position is
not sensitive – which is recorded on the recipe, so the decision is
visible later. A column that looks like a coordinate only by the shape
of its values raises a warning instead of stopping, because that
detection is a guess and a false positive should not block a legitimate
mask.
...
Where a site’s recorded coordinates differ slightly between rows, name
the site columns and the group is consolidated to its centroid.
...
estimator and leaves the full displacement in place. Choose the unit
carefully: it is the finest grouping denoting one physical place at one
time, so for a multi-year trial series it is location by year rather
than location alone, because trials at one named location in different
seasons legitimately sit in different paddocks.
...
a clone carries the account of its own coordinate structure.
...
A coordinate declared to `coords` is retained in the synthetic, so
[`audit_mask()`](https://max578.github.io/masque/reference/audit_mask.md)
reports it – retention is worth knowing about. It reports it as
